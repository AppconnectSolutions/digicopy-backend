...
import { pool } from "../db.js"; // ✅ matches your db.js export
...
router.post("/register", async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      customer: { id: result.insertId, name, email, mobile, tier: "Silver", points_balance: 0 },
    });
  } catch (err) {
...
router.post("/upload", upload.single("file"), async (req, res) => {
  ...
  stream.on("data", (row) => {
    const mobileStr = row.mobile?.toString();
    customers.push({
      name: row.name,
      email: row.email,
      mobile: mobileStr,
      password: generateDefaultPassword(mobileStr), // ✅ UNCHANGED
    });
  });
  ...
  stream.on("end", async () => {
    try {
      for (const c of customers) {
        if (!c.name || !c.email || !c.mobile) continue;
        ...
        const hashedPassword = await bcrypt.hash(c.password, 10); // ✅ UNCHANGED
      ...
      return res.json({ message: "Customers uploaded successfully" });
    } catch (err) {
      console.error("Upload error:", err);
      return res.status(500).json({ message: "Customer upload failed" });
    }
  });
...
/* ------------------- GET ALL CUSTOMERS ------------------- */
/* ------------------- GET ALL CUSTOMERS (WITH XEROX COUNT) ------------------- */
...
router.post("/admin-add", async (req, res) => {
  try {
    ...
    const [exists] = await pool.query("SELECT id FROM customers WHERE mobile=?", [mobile]);
    if (exists.length > 0)
      return res.status(409).json({ message: "Customer already exists" });

    const defaultPassword = mobile.toString().slice(-4);
    const hashedPassword = await bcrypt.hash(defaultPassword, 10);

    const [result] = await pool.query(
      `INSERT INTO customers (name, email, mobile, password_hash, tier, points_balance)
