...
async function backupDatabaseToGoogleSheet() {
  // ✅ Prevent double-run (important if you deploy multiple instances)
  const [lockRows] = await pool.query("SELECT GET_LOCK('gsheet_backup_lock', 0) AS got");
  if (lockRows?.[0]?.got !== 1) return; // someone else running
    ...
    async () => {
      try {
        ...
        if (now >= next) {
          ...
          // ✅ schedule next exactly 7 days from NOW
          await setSetting("next_backup_at", addDaysIso(nowIso, 7));
        }
      } catch (e) {
        console.error("Backup watcher failed:", e);
      }
    },
...
export async function getBackupStatus() {
  ...
  return { enabled, last_backup_at: last || null, next_backup_at: next || null };
}
...
export async function setBackupEnabled(enabled) {
  ...
  // ✅ Next backup exactly after 7 days from NOW
  await setSetting("next_backup_at", addDaysIso(nowIso, 7));

  return { ok: true, enabled: true, next_backup_at: addDaysIso(nowIso, 7) };
}
...
export async function runBackupNow() {
  ...
  const nowIso = new Date().toISOString();
  await setSetting("last_backup_at", nowIso);
  await setSetting("next_backup_at", addDaysIso(nowIso, 7)); // reset weekly cycle from now
}
